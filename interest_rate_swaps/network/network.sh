...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(docker run --rm hyperledger/fabric-tools:2.2 peer version | sed -ne 's/ Version: //p' | head -1)
...
function networkDown() {
  ...
  docker run -v "$PWD:/tmp/first-network" --rm hyperledger/fabric-tools:2.2 rm -Rf /tmp/first-network/ledgers-backup
